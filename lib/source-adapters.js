...
function parseNlaEmbeddedWork(document) {
  const scriptText = Array.from(document.querySelectorAll("script"))
    .map((script) => script.textContent || "")
    .find((text) => /var\s+work\s*=\s*JSON\.parse\(JSON\.stringify\(/.test(text));
  const match = scriptText?.match(/var\s+work\s*=\s*JSON\.parse\(JSON\.stringify\(([\s\S]*?)\)\);/);
  if (!match) {
    return null;
  }
  try {
...
function nlaMarcFields(work) {
  const fields = [];
  const records = Array.isArray(work?.marcData?.record) ? work.marcData.record : [];
  for (const record of records) {
    for (const datafield of Array.isArray(record?.datafield) ? record.datafield : []) {
      const tag = String(datafield?.tag || "");
      const subfields = Array.isArray(datafield?.subfield) ? datafield.subfield : [datafield?.subfield].filter(Boolean);
      const value = subfields.map((entry) => cleanText(entry?.content || "")).filter(Boolean).join(" ");
      if (tag && value) {
        fields.push({ label: `MARC ${tag}`, value });
      }
    }
  }
  return fields;
...
function extractNlaDigitalObject(document, currentUrl) {
  const url = new URL(currentUrl);
  const pathname = url.pathname;
  if (url.hostname.toLowerCase() !== "nla.gov.au" || !/\/nla\.obj-\d+(?:\/|$)/i.test(pathname)) {
    return { supported: false };
  }

  const objectMatch = pathname.match(/\/(nla\.obj-\d+)/i);
  const objectId = objectMatch?.[1] || "";
  const work = parseNlaEmbeddedWork(document);
  const pid = cleanText(work?.pid || objectId);
  if (!pid) {
    return { supported: false };
  }
  ...
  const thumbnailUrl = normalizeUrl(
    pickMeta(document, ['meta[property="og:image"]', 'meta[property="og:image:url"]', 'meta[name="twitter:image"]']) ||
  ...
  const sourceTitle = work?.collection ? `National Library of Australia: ${work.collection}` : "National Library of Australia";
  ...
  const marcFields = nlaMarcFields(work).slice(0, 20);
  const description =
    metadataFields.find((field) => field.label === "Extent")?.value ||
    pickMeta(document, ['meta[name="description"]', 'meta[property="og:description"]']) ||
    "";
  const accessCopy = Array.isArray(work?.copies) ? work.copies.find((copy) => copy?.copyrole === "access") : null;
  ...
  return {
    ...
    title: title || pid,
    url: objectUrl,
    aliases: uniqueValues([objectUrl, viewerUrl, currentUrl, imageUrl, thumbnailUrl], currentUrl),
    citation: `${title || pid}. National Library of Australia. ${objectUrl}`,
    description,
    fullText: "",
    sourceTitle,
    imageUrl,
    ...
    metadataFields: [...metadataFields, ...marcFields],
    rawMetadata: cleanText(JSON.stringify({
      pid,
...
function getMetadataDescription(metadataFields) {
  ...
  for (const preferredLabel of preferredLabels) {
    const field = normalized.find((entry) => cleanText(entry?.label || "").toLowerCase() === preferredLabel);
    if (field?.value) {
...
function extractAgwa(document, currentUrl) {
  const url = new URL(currentUrl);
  const hostname = url.hostname.toLowerCase();
  const pathname = url.pathname.toLowerCase();
  ...
  for (const row of Array.from(document.querySelectorAll(".detail-row"))) {
    const label = cleanText(row.querySelector(".detail-label")?.textContent || "");
    const value = cleanText(row.querySelector(".detail-value")?.textContent || "");
    if (label && value) {
      metadataFields.push({ label, value });
    }
  }
  const byLabel = new Map(metadataFields.map((field) => [field.label.toLowerCase(), field.value]));
  const objectId = pathname.match(/\/objects\/(\d+)\//i)?.[1] || "";
  const accessionNumber = byLabel.get("accession number") || "";
  const artist = byLabel.get("artist/maker and role") || cleanText(document.querySelector("h2")?.textContent || "");
  const date = byLabel.get("date") || "";
  const medium = byLabel.get("medium") || "";
  const collection = metadataFields.find((field) => /state art collection/i.test(field.value))?.value || "Art Gallery of Western Australia";
  const imageUrl = [
    ...
    .map((candidate) => normalizeUrl(candidate, currentUrl))
    .find((candidate) => /\/records\/images\//i.test(candidate)) || "";
  const subjectTags = cleanText(
    Array.from(document.querySelectorAll(".tag-cloud, .subject-tags, [class*='tag']"))
      .map((node) => node.textContent || "")
      .join(" ")
  );
  const description = [artist, date, medium].filter(Boolean).join("; ");
  ...
  return {
    ...
    type: imageUrl ? "image" : "text",
    id: accessionNumber || objectId || slugify(title) || "agwa-record",
    title,
    url: normalizeUrl(currentUrl, currentUrl),
    aliases: uniqueValues([currentUrl, imageUrl], currentUrl),
    citation: `${title}. ${collection}. ${normalizeUrl(currentUrl, currentUrl)}`,
    sourceTitle: collection,
    description,
    fullText: "",
    imageUrl,
    metadataFields,
    rawMetadata: cleanText([document.querySelector(".about-the-work")?.textContent, subjectTags, document.body?.textContent].join(" ")).slice(0, 4000)
  };
}

function extractItemFromHtml(url, html) {
  const document = buildDom(html, url);
  const extractors = [extractNlaDigitalObject, extractTrove, extractSlwa, extractWaMuseum, extractAgwa];
